...
const coreLeadCaptureApi = axios.create({
  //prod
  baseURL: "http://192.200.12.174:7075",
  // baseURL: "http://10.8.20.38:7075",

  timeout: 50000,
});
// Request interceptor to add the Authorization header
coreLeadCaptureApi.interceptors.request.use(
  async (config) => {
...
coreLeadCaptureApi.interceptors.response.use(
  (response) => response,
  async (error: AxiosError) => {
    ...
    if (error.response?.status === 401 && !originalRequest?._retry) {
      ...
      if (newToken) {
        ...
        return coreLeadCaptureApi(originalRequest);
      }
    }
...
export default coreLeadCaptureApi;

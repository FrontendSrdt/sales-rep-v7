...
import ReCalculateInput from "./re-calculate/ReCalculateInput";
import ReCalculateInstallment from "./re-calculate/ReCalculateInstallment";
...
const OfferAnalysis: React.FC<OfferAnalysisProps> = ({ ongetLeadOfferHistory, leadStatus, isEnablePackageDeal, onEnableOfferGrantHandler }) => {
  ...
  const { isLoading: isLoadingForFeeCalculation } = useSelector((state: RootState) => state.getFeeCalculationByProgramIdResponse);
  ...
  // console.log("leadOfferHistoryByOfferIdResponse==", leadOfferHistoryByOfferIdResponse);
  // console.log("getLeadOfferByLeadIdResponse==", getLeadOfferByLeadIdResponse.length);
  // const onEnableOfferGrantHandler = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      {!isLoadingForFeeCalculation && (
        <div className="py-3 px-3">
          ...
          <div className="mt-4">
            ...
            <>
              {/* When History table is not exit  */}
              {getLeadOfferByLeadIdResponse.length === 0 ? (
                <>
              ...
              ) : getLeadOfferByLeadIdResponse.length !== 0 && getLeadOfferByLeadIdResponse.length === 1 && leadStatus === "submitted" ? (
                <>
                  {findLeadScholarshipDetailsResponse.scholarshipCategoryId === 7 &&
                    findLeadScholarshipDetailsResponse.scholarshipSchemeId === 24 &&
                    findLeadScholarshipDetailsResponse.coreScholarshipSlabId === 74 && (
                ...
                getLeadOfferByLeadIdResponse.length > 1 &&
                (leadStatus === "re-issued" || leadStatus === "accept" || leadStatus === "submitted") &&
                ...
                <>
                  {findLeadScholarshipDetailsResponse.scholarshipCategoryId === 7 &&
                    ...
                    findLeadScholarshipDetailsResponse.coreScholarshipSlabId === 74 && (
                      <>
                        {isLoadingForLeadOfferDetails && <p className="mt-4 px-5">Loading...</p>}

                        {!isLoadingForLeadOfferDetails && <HistoryInstallmentAndOfferAnalysis />}
                      </>
                    )}
                </>
              ) : (
